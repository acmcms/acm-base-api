...
public final class ControlSAPI {
	 ...
	 *            <li>BaseFunction returning one of above while having 'target' as 'this' and
	 *            command_id as its first argument</li>
	 *            </ol>
	 * @return form */
	public static ControlForm<?> createSimpleForm(final BaseObject title, final BaseObject target, final ControlFieldset<?> fieldset, final BaseObject result) {
	 ...
	 * @return commandset */
	@Deprecated
	public static ControlCommandset filterAccessible(final ExecProcess ctx, final String path, final ControlCommandset options) {
	 ...
	 * @return node array */
	@Deprecated
	public static List<ControlNode<?>> filterAccessible(final ExecProcess ctx, final String basePath, final List<ControlNode<?>> nodes) {
	 ...
	 * @param options
	 * @return commandset */
	public static ControlCommandset filterAccessibleCommands(final ExecProcess ctx, final String path, final ControlCommandset options) {
	 ...
	 * @param nodes
	 * @return node array */
	public static List<ControlNode<?>> filterAccessibleHierarchy(final ExecProcess ctx, final String basePath, final List<ControlNode<?>> nodes) {
		...
		for (int i = 0; i < length; ++i) {
			...
			final boolean deny;
			{
				...
				if (manager.securityCheck(level, check1, "$view_tree") != null) {
					...
					deny = check2 == null || check1.equals(check2) || manager.securityCheck(level, check2, "$view_tree") == null;
				}
			}
			if (result == null) {
				if (deny) {
					/** first denied, now we have to create array and put all entries before current
					 * one */
				...
				/** no need to add current element, it is either denied or we expect returning
				 * unmodified list */
			} else {
				if (!deny) {
					result.add(node);
	 ...
	 * @param end
	 * @return list */
	public static List<ControlBasic<?>> filterByFieldDate(final List<ControlBasic<?>> listing, final String fieldName, final long start, final long end) {

		if (listing == null || fieldName == null || start == -1L && end == -1L || listing.isEmpty() || fieldName.length() == 0) {
			return listing;
		}
	 ...
	 * @param ignoreSuffix
	 * @return list */
	public static List<ControlBasic<?>> filterByFieldString(final List<ControlBasic<?>> listing,
		...
		if (listing == null || fieldName == null || pattern == null || listing.isEmpty() || fieldName.length() == 0 || pattern.length() == 0) {
			return listing;
		}
		final List<ControlBasic<?>> result = new ArrayList<>();
		if (ignoreCase) {
	 ...
	 * @param command
	 * @return boolean */
	public static boolean isAccessibleCommand(final ExecProcess process, final String path, final ControlCommand<?> command) {
	 ...
	 * @param descending
	 * @return list */
	public static List<ControlBasic<?>> sortByFieldDate(final List<ControlBasic<?>> listing, final String fieldName, final boolean descending) {
	 ...
	 * @param descending
	 * @return list */
	public static List<ControlBasic<?>> sortByFieldNumeric(final List<ControlBasic<?>> listing, final String fieldName, final boolean descending) {
	 ...
	 * @param descending
	 * @return list */
	public static List<ControlBasic<?>> sortByFieldString(final List<ControlBasic<?>> listing, final String fieldName, final boolean ignoreCase, final boolean descending) {
	...
	public static String toLine(final Object object) {
		...
		if (object instanceof BaseMessage) {
			...
			final String type = message.isCharacter()
				? "text message, " + Format.Compact.toDecimal(((BodyAccessCharacter) message).getCharacterContentLength()) + " chars"
				: message.isBinary()
					? "binary message, " + Format.Compact.toBytes(((BodyAccessBinary) message).getBinaryContentLength()) + " bytes"
					: message.isFile()
						? "file message, " + Format.Compact.toBytes(message.getFile().length()) + " bytes"
						: message.isEmpty()
							? "empty message"
							: "other message";
