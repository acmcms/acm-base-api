	...
	final Cache3Entry getValue() {
		...
		synchronized (this) {
			...
			try {
				 ...
				 * so-called spurious wakeup. While this will rarely occur in practice, applications
				 * must guard against it by testing for the condition that should have caused the
				 * thread to be awakened, and continuing to wait if the condition is not satisfied.
				 ...
				 * Programming in Java (Second Edition)" (Addison-Wesley, 2000), or Item 50 in
				 * Joshua Bloch's "Effective Java Programming Language Guide" (Addison-Wesley,
				 * 2001). */
				for (//
