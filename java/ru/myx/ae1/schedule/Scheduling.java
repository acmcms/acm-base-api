...
public class Scheduling implements ModuleSchedule {
	...
	public static final Scheduling getScheduling(final StorageImpl parent, final String tnQueue, final String tnLog) {
		...
		try (final Connection conn = parent.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement("SELECT count(name) FROM " + tnQueue, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
	...
	static final BaseObject getBlobAsMap(final byte[] bytes) throws Exception {
		...
		return Xml.toMap("blobToMap(scheduling)", Transfer.wrapCopier(bytes), StandardCharsets.UTF_8, null, new BaseNativeObject(), null, null);
	}
	...
	public void start() {
		...
		synchronized (this) {
			if (this.schedulerTask == null) {
				final ExecProcess process = Exec.createProcess(this.parent.getServer().getRootContext(), "scheduler context");
				...
				Act.later(process, this.schedulerTask = new SchedulerTask(this.parent, false, this.tnQueue, this.tnLog), 20_000L);
			}
		}
	}
			...
			try {
				...
				if (deleted != null && deleted.length > 0) {
					try (final PreparedStatement ps = conn
							.prepareStatement("DELETE FROM " + this.tnQueue + " WHERE objectid=? AND scheduleid in ('" + Text.join(deleted, "','") + "')")) {
				...
				if (created != null && created.length > 0) {
					for (final Action<?> action : created) {
						try (final PreparedStatement ps = conn.prepareStatement(
								"INSERT INTO " + this.tnQueue + "(scheduleid,objectid,systemid,state,ownerid,schedule,name,command,parameters) VALUES (?,?,?,?,?,?,?,?,?)")) {
				...
				Report.exception(Scheduling.OWNER, "Error commiting a change: storageId=" + this.parent.getMnemonicName() + ", objectid=" + objectId, t);
	...
	Action<?>[] getQueue(final String objectId) {
		...
		try {
			try (final Connection conn = this.parent.nextConnection()) {
				try (final PreparedStatement ps = conn
						.prepareStatement("SELECT scheduleid,state,ownerid,schedule,name,command,parameters FROM " + this.tnQueue + " WHERE objectid=? ORDER BY schedule ASC")) {
					ps.setString(1, objectId);
					try (final ResultSet rs = ps.executeQuery()) {
						if (rs.next()) {
							...
							do {
								...
								final Action<?> action = new Action<>(scheduleid, state, schedule.getTime(), ownerid, name, command, data);
								result.add(action);
							} while (rs.next());
